...
describe("AppMenu", () => {
	it("hangs the menu on the brand rather than adding a control to the bar", () => {
		renderTopBar("Demo Project");
		const trigger = screen.getByRole("button", { name: /PL Demo Studio/ });
	...
	it("opens on click and offers shortcuts, AI settings and about", () => {
		renderTopBar("Demo Project");
		fireEvent.click(screen.getByRole("button", { name: /PL Demo Studio/ }));
...
describe("EditorTopBar responsive affordances and tooltips", () => {
	...
	it("keeps the brand trigger accessible by label and title even when text collapses", () => {
		renderTopBar("Demo Project");
		const brandBtn = screen.getByRole("button", { name: "PL Demo Studio" });
		expect(brandBtn).toHaveAttribute("title", "PL Demo Studio");
		expect(brandBtn).toHaveAttribute("aria-label", "PL Demo Studio");
	});
