...
## Automatic browser bounds and larger picker

The source selector now opens at 94% of the current display work-area width and 92% of its height, capped at 1440 by 1000. It is resizable. The page preview uses the main pane, with settings beside it. Source thumbnails request 1920 by 1200 pixels. Existing manual crops survive reopening.

On entry, the picker requests a crop suggestion from the WGC helper's new `--detect-browser-area` command. The helper uses Windows UI Automation Document bounds from a supported browser process and converts physical desktop bounds through DWM frame bounds. It reads geometry only, with a six-second process timeout. Visible long scrollbars at the document's right or bottom edge are removed when exposed. Nested table scrollbars remain. No image recognition, browser extension, URL reading, or automatic browser control is involved. Unsupported or missing accessibility bounds leave manual selection available; an unfinished response cannot overwrite manual adjustments.

Supported process names are Chrome, Edge, Firefox, Brave, Vivaldi, Opera, and Arc; this is eligibility, not verified compatibility. Hidden, overlay, or custom scrollbars may not expose usable bounds. The user reviews the suggestion before applying it. Browser changes during capture still require a new selection.

Validation: rebuilt Windows helper and its native GPU crop/audio tests passed. Both TypeScript checks, lint (24 baseline warnings), localization keys, and production renderer build passed. The full unit run passed 223 files; two failures in the new helper test's cleanup callback were fixed, then all three helper tests passed. A hidden built-picker test at 1440 by 900 verified automatic suggestion, manual override, and source payload with no page errors or horizontal overflow. Its preview exceeded 850 by 550 pixels. These are synthetic UI bounds, not a live browser detection test. Live browser accessibility detection, WGC alignment, and scrollbar exclusion remain unverified. No visible windows were opened.

API reference: https://learn.microsoft.com/en-us/windows/win32/winauto/uiauto-supportdocumentcontroltype
